...
try:
    ...
    print("Asegurate de que 'src' contenga las carpetas 'ocr' y 'deteccion'.")
    sys.exit(1)
...
def process_video_stream(conn):
    ...
    try:
        ...
        print(f"Se perdio la conexion con la Pi: {e}")
    except Exception as e:
        print(f"Error en el hilo de procesamiento: {e}")
        import traceback
        traceback.print_exc()
    finally:
        print("Cerrando conexion con la Pi.")
        conn.close()
...
def start_pi_listener():
    ...
    while True:
        try:
            ...
            print(f"Error al aceptar conexion de la Pi: {e}")
    server_socket.close()
