...
class AnalizadorEscena:
    """
    Evolución de DetectorObjetos.
    Esta clase integra la detección de objetos prioritarios con la segmentación
    de contexto ('stuff') para generar una descripción completa de la escena.
    """
    def __init__(self, modelo_det_path: str = 'yolov8n.pt', modelo_seg_path: str = 'yolov8n-seg.pt', confianza_minima: float = 0.5):
        ...
        #simplificacion
        self.objetos_prioritarios = { 0, 2, 3, 5, 24, 39, 41, 56, 57, 58, 59, 60, 61, 62, 63, 65, 67, 73 }
        #TRABAJAR AQUI, los nuevos set
        nuevas_etiquetas = {
            80: 'escaleras',
            81: 'puerta', 
            82: 'extintor',
            83: 'señal_piso_mojado',
            84: 'pared', 

        }
        self.objetos_prioritarios.update({80, 81, 82, 83, 84})
    ...
    def _generar_descripcion_completa(self, objetos: List[Dict], contexto: List[str]) -> str:
        if not objetos and not contexto:
            return "No se detectan elementos claros."
        desc_objetos = ""
        if objetos:
            if len(objetos) == 1:
                det = objetos[0]
                desc_objetos = f"Veo {det['nombre']} {det['distancia_relativa']} {det['posicion']}."
            else:
                nombres = [f"{d['nombre']} {d['posicion']}" for d in objetos]
                desc_objetos = f"Veo varios objetos: {', '.join(nombres[:2])} y posiblemente otros más."

        desc_contexto = ""
        if contexto:
            if len(contexto) == 1:
                desc_contexto = f" En el entorno hay {contexto[0]}."
            else:
                desc_contexto = f" El entorno parece tener {', '.join(contexto[:-1])} y {contexto[-1]}."

        return (desc_objetos + desc_contexto).strip()
    ...
    def dibujar_analisis(self, imagen: np.ndarray, analisis: Dict) -> np.ndarray:
        ...
        cv2.putText(frame_resultado, descripcion, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv2.LINE_AA)
